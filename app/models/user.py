...
class User(AbstractUser):
    ...
    def total_avg_vote_given(self):
        avg_vote_given = (
            self.votes.aggregate(avg_score=Avg("score")).get("avg_score") or 0
        )
    ...
    def total_avg_vote_received(self):
        avg_vote_received = (
            self.memes.aggregate(avg_score=Avg("votes__score")).get("avg_score") or 0
        )
    ...
    def total_avg_vote_received_from_self(self):
        avg_own_vote = (
            self.memes.filter(votes__user=self)
            .aggregate(avg_score=Avg("votes__score"))
            .get("avg_score")
            or 0
        )
    ...
    def total_avg_voting_time(self):
        avg_voting_time = sum(v.voting_time for v in self.votes.all()) / (
            self.votes.count() or 1
        )
    ...
    def competitions_won(self):
        ...
        for participant in self.participants.all():
            if (
                participant.competition.winning_meme
                and participant.competition.winning_meme.user == self
            ):
                win_ctr += 1
    ...
    def highest_rated_user(self):
        return (
            User.objects.annotate(
                avg_score=Avg(
                    Case(
                        When(memes__votes__user=self, then=F("memes__votes__score")),
                        output_field=FloatField(),
                    )
                )
            )
            .filter(memes__votes__user=self)
            .exclude(id=self.id)
            .order_by("-avg_score")
            .first()
        )
